...
public class InvokeFromOrg4 implements Runnable {
	...
	public InvokeFromOrg4(String threadName, Date s)
	{
	...
	public void run() {
		...
		try {
            Util.cleanUp();
			String caUrl = Config.CA_ORG4_URL;
			CAClient caClient = new CAClient(caUrl, null);
			// Enroll Admin to ORG4MSP
			UserContext adminUserContext = new UserContext();
			adminUserContext.setName(Config.ADMIN);
			adminUserContext.setAffiliation(Config.ORG4);
			adminUserContext.setMspId(Config.ORG4_MSP);
			...
			Peer peer = fabClient.getInstance().newPeer(Config.ORG4_PEER_0, Config.ORG4_PEER_0_URL);
			EventHub eventHub = fabClient.getInstance().newEventHub("eventhub01", "grpc://localhost:10053");
